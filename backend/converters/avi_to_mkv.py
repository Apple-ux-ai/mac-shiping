...
class AVIToMKVConverter(BaseConverter):
    ...
    def convert(self, input_path: str, output_dir: str, **options) -> dict:
        ...
        try:
            ...
            # Actually, in the previous modules we mapped 1 as low quality, 35 as high.
            # For CRF: 18 is high quality, 28 is lower quality.
            # Mapping: crf = 28 - int((quality_percent - 1) * (28 - 18) / 34)
            ...
            # Video encoding settings for MKV (libx264)
            ...
            # Audio encoding settings for MKV (AAC)
            if has_audio:
            ...
            if not result.get('success'):
                ...
                if "Stream map" in result.get('error', '') and audio_track != 0:
                    command = ['ffmpeg', '-y', '-i', input_path]
                    command.extend(['-c:v', 'libx264', '-crf', str(crf), '-preset', 'medium'])
