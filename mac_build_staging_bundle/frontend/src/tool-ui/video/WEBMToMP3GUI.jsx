...
const WEBMToMP3GUI = ({ onBack }) => {
  ...
  const [fileSettings, setFileSettings] = useState({}); // { [filePath]: { startTime, endTime, audioTrack, audioBitrate } }
  ...
  const handleConvert = async () => {
    ...
    let successCount = 0;
    let failCount = 0;
    let processedCount = 0; // 实际进入后端转换的文件数
    ...
    try {
        if (api.isAvailable()) {
            for (let index = 0; index < selectedFiles.length; index++) {
                ...
                processedCount++;
                const result = await api.convert('convert-webm-to-mp3', {
                    sourcePath: file,
                    outputDir: outputDir,
                    params: { 
                        audioBitrate, 
  ...
  const handleCancelConfirm = async () => {
    ...
    if (!targetPath && currentFileRef.current && lastOutputDir) {
      const pathParts = currentFileRef.current.split(/[\\/]/);
      const filenameWithExt = pathParts[pathParts.length - 1];
      const filename = filenameWithExt.replace(/\.[^/.]+$/, "");
      
      const sep = currentFileRef.current.includes('\\') ? '\\' : '/';
      targetPath = `${lastOutputDir}${sep}${filename}.mp3`;
    }
  ...
  const activeFileInfo = activeFile ? fileInfos[activeFile] : null;
  const trackCount = activeFileInfo?.audio_tracks_count || 1;
  const trackOptions = Array.from({ length: trackCount }, (_, index) => ({
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '8px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>WEBM To MP3 转换器</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px' }}>从 WEBM 视频中提取高质量 MP3 音频，支持批量转换和音频质量设置。</div>
        </div>
      </div>
        ...
        <div className="tool-main-content">
          <div className="tool-left-panel">
            <FileUploaderV2 
                ...
                showAudioInfo={true}
                uploadPlaceholder="将您的WEBM文件拖拽到此处"
