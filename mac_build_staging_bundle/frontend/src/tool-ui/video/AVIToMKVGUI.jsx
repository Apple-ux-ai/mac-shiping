  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        const result = await api.convert('convert-avi-to-mkv', {
          sourcePath: filePath,
          outputDir: outputDir,
          params: {
            quality,
    ...
    try {
      ...
      if (Array.isArray(currentTargetsRef.current) && currentTargetsRef.current.length > 0) {
        ...
        if (outputDir) {
          ...
          targetPath = `${outputDir}${sep}${filename}.mkv`;
        }
      }
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '8px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>AVI To MKV 转换器</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px' }}>一款专业工具，可将AVI 视频文件转换为MKV视频格式，并支持自定义编码参数。</div>
        </div>
      </div>
