...
const PRESET_CONFIGS = {
  '低质量': { quality: 30, audioBitrate: '96k', resolution: '854x480', useVBR: true },
  '中等质量': { quality: 70, audioBitrate: '128k', resolution: '1280x720', useVBR: true },
  '高质量': { quality: 90, audioBitrate: '320k', resolution: '1920x1080', useVBR: true },
  '社交媒体': { quality: 60, audioBitrate: '192k', resolution: '1080x1920', useVBR: true }
};
...
const MP4ToWEBMGUI = ({ onBack }) => {
  const navigate = useNavigate();
  // State for settings
  const [quality, setQuality] = useState(70); // Default quality percent
  ...
  const handleRemoveFile = (index) => {
    ...
    if (activeFile === fileToRemove) {
      setActiveFile(newFiles[index] || newFiles[index - 1] || null);
    }
  ...
  const handleConvert = async () => {
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      ...
      try {
        const result = await api.convert('convert-mp4-to-webm', {
          sourcePath: file,
          outputDir: outputDir,
          params: {
            quality,
        ...
        if (!isCancelledRef.current) {
          ...
          applyConversionNotificationRule({
            ...
            ui: { showAlert },
            data: {
              errorMessage: `处理文件 ${file} 时出错: ${err.message}`,
              customTitle: '错误',
              customMessage: `处理文件 ${file} 时出错: ${err.message}`
            }
          });
          break;
        }
    ...
    if (!isCancelledRef.current && !hasError) {
      setConvertProgress(0);
      ...
      setTimeout(() => {
        applyConversionNotificationRule({
          ...
          data: {
            customTitle: '完成',
            customMessage: `成功转换 ${selectedFiles.length} 个文件！`,
            buttonText: '确定',
            onConfirm: () => {
              setAlertModal(prev => ({ ...prev, isOpen: false }));
            },
    ...
    try {
      ...
      if (Array.isArray(currentTargetsRef.current) && currentTargetsRef.current.length > 0) {
        ...
        if (baseDir) {
          ...
          const sep = window.navigator.userAgent.includes('Windows') ? '\\' : '/';
          targetPath = `${baseDir}${sep}${filename}.webm`;
        }
      }
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        <div className="tool-breadcrumbs">
          ...
          <span style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <span className="current">MP4 To WEBM</span>
          </span>
        </div>
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>MP4 To WEBM 转换器</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>—— 专业级 MP4 转 WEBM 工具，支持批量转换和高质量 WebM 格式输出。</div>
        </div>
      </div>
            ...
            <SettingsPanel title="转换选项">
              ...
              <div className="settings-group">
                ...
                <LocalSettingSelect
                  ...
                  onChange={(val) => handleSettingChange(setAudioTrack, Number(val))}
                  disabled={isConverting}
                />
              </div>
          ...
          <div className="confirm-body" style={{ padding: '24px', background: 'var(--card-bg)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
              ...
              <div className="setting-group">
                ...
                <div className="mode-toggle-group" style={{ display: 'flex', gap: '4px', marginBottom: '16px', background: 'var(--bg-secondary)', padding: '4px', borderRadius: '8px' }}>
                  <button 
                ...
                <div className="compression-mode-content" style={{ minHeight: '80px' }}>
                  {useVBR ? (
                    ...
                    <div className="setting-sub-item">
                      ...
                      <select
                        className="custom-select"
                        style={{ width: '100%', height: '36px', borderRadius: '8px', border: '1px solid var(--border-color)', background: 'var(--card-bg)', color: 'var(--text-primary)' }}
                        value={videoBitrate}
                        onChange={(e) => {
