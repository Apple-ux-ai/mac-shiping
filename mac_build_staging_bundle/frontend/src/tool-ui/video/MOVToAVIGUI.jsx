    ...
    try {
      if (api.isAvailable()) {
        for (let index = 0; index < selectedFiles.length; index++) {
          ...
          const result = await api.convert('convert-mov-to-avi', {
            sourcePath: file,
            outputDir: outputDir,
            params: {
              quality,
  ...
  const handleCancelConfirm = async () => {
    ...
    if (targetPaths.length === 0 && currentFileRef.current) {
      ...
      if (outDir) {
        ...
        targetPaths = [`${outDir}${sep}${filename}.avi`];
      }
    }
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        <div className="tool-breadcrumbs">
          ...
          <span style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <span className="current">MOV To AVI</span>
          </span>
        </div>
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>MOV To AVI 转换器</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>—— 专业级 MOV 转 AVI 工具，支持批量转换和高质量输出。</div>
        </div>
      </div>
