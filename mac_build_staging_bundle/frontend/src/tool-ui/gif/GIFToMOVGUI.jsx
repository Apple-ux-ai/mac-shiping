...
const GIFToMOVGUI = ({ onBack }) => {
  ...
  const processFiles = async (files) => {
    ...
    // Filter by extension (optional but good practice)
    ...
    try {
      if (api.isAvailable()) {
        for (let index = 0; index < selectedFiles.length; index++) {
          ...
          const result = await api.convert('convert-gif-to-mov', {
            sourcePath: file,
            outputDir: outputDir,
            params: {
              fps
        ...
        if (!isCancelledRef.current) {
          ...
          applyConversionNotificationRule({
            ...
            data: {
              customTitle: '完成',
              customMessage: '所有 GIF 已成功转换为 MOV 文件！',
              buttonText: '确定',
  ...
  const confirmCancel = async () => {
    ...
    if (Array.isArray(currentTargetsRef.current) && currentTargetsRef.current.length > 0) {
      ...
      if (outputDir) {
        ...
        const base = `${outputDir}${sep}${filename}.mov`;
        const candidates = [base];
        for (let i = 1; i <= 50; i++) {
          candidates.push(`${outputDir}${sep}${filename}_${i}.mov`);
        }
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '8px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>GIF To MOV 转换器</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px' }}>一款专业工具，可将 GIF 动图转换为 MOV 视频格式，并支持自定义参数。</div>
        </div>
      </div>
