...
const GIFToPNGGUI = ({ onBack }) => {
  ...
  const PRESET_CONFIGS = {
    '低质量': { quality: 60, interval: 50 },
    '中等质量': { quality: 80, interval: 80 },
    '高质量': { quality: 90, interval: 100 },
    '社交媒体': { quality: 85, interval: 100 }
  };
  ...
  const handleFileSelect = async () => {
    if (api.isAvailable()) {
      const result = await api.openFileDialog(['gif']);
      if (result) {
  ...
  const confirmCancel = async () => {
    ...
    let targetPaths = [];
    if (currentTargetsRef.current) {
      if (Array.isArray(currentTargetsRef.current)) {
    ...
    const handleProgress = (data) => {
      ...
      if (data && data.type === 'output') {
        ...
        if (Array.isArray(data.targets) && data.targets.length > 0) {
          ...
          // Prefer ZIP if available, otherwise folder
          const zip = data.targets.find(t => t.endsWith('.zip'));
          output = zip || data.targets[0];
        } else if (data.output) {
    ...
    try {
      if (api.isAvailable()) {
        for (let index = 0; index < selectedFiles.length; index++) {
          ...
          const result = await api.convert('convert-gif-to-png', {
            sourcePath: file,
            outputDir: outputDir,
            params: {
              quality,
        ...
        if (!isCancelledRef.current) {
          ...
          applyConversionNotificationRule({
            ...
            data: {
              customTitle: '完成',
              customMessage: '所有 GIF 已成功转换为 PNG 序列并打包为 ZIP！',
              buttonText: '确定',
              onConfirm: async () => {
                setAlertModal(prev => ({ ...prev, isOpen: false }));
              },
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        <div className="tool-breadcrumbs">
          <span style={{ display: 'flex', alignItems: 'center' }}>
            <a onClick={onBack}>GIF 转换器</a>
          </span>
          <span style={{ display: 'flex', alignItems: 'center' }}>
            <span style={{ margin: '0 8px', color: 'var(--text-secondary)' }}>/</span>
            <span className="current">GIF To PNG</span>
          </span>
        </div>
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>GIF To PNG 转换器</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>一款专业工具，可将 GIF 动图转换为 PNG 图像序列，并支持自定义参数。</div>
        </div>
      </div>
