...
class WEBMToPNGConverter(BaseConverter):
    """WEBM to PNG Converter"""
    ...
    def convert(self, input_path: str, output_dir: str, **options) -> dict:
        """
        Convert WEBM to PNG sequence and zip it
        """
        try:
            ...
            # Apply trimming before input for fast seeking if possible, or after input
            ...
            # Map quality (1-35) to compression level (0-9)
            # Higher quality value in UI usually means better quality, but for PNG it's compression.
            # If 35 is best quality, it might mean lower compression or just a generic slider.
            # Let's map it roughly: 35 -> 9 (max compression), 1 -> 1.
            ...
            # Report 99% before zipping to keep UI alive
            print(json.dumps({"type": "progress", "percent": 99}))
            sys.stdout.flush()

            try:
                # Optimized ZIP: use lower compression level or just store if files are already compressed PNGs
                # to speed up the "100% stuck" phase
