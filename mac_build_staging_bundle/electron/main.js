...
ipcMain.handle('check-update', async (event, payload) => {
  ...
  ipcMain.on('start-update', (event, updateInfo) => {
    ...
    try {
      // Use 'start' command via exec to ensure UAC prompt and avoid EACCES
      const args = [
        '--url', url,
        '--hash', hash,
        '--dir', appDir,
      ...
      // Quote arguments carefully for cmd
      ...
      // Quit app to allow updater to overwrite files
...
function startPythonBackend() {
  ...
  if (app.isPackaged) {
    ...
    const scriptPath = path.join(process.resourcesPath, 'backend', 'api.exe');
    command = scriptPath;
    args = [];
