    ...
    try {
        if (api.isAvailable()) {
            for (let index = 0; index < selectedFiles.length; index++) {
                ...
                const result = await api.convert('convert-webm-to-mp4', {
                  sourcePath: file,
                  outputDir: outputDir,
                  params: { 
                    quality, 
  ...
  const handleCancelConfirm = async () => {
    ...
    if (api.isAvailable()) {
      ...
      if (currentFileRef.current) {
          ...
          if (outputDir) {
             fallbackPath = `${outputDir}/${fileName}.mp4`;
          }
      }
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '8px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>WEBM To MP4 转换器</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px' }}>专业级 WEBM 转 MP4 工具，支持批量转换和高质量输出。</div>
        </div>
      </div>
