...
const AVIToH264GUI = ({ onBack }) => {
  ...
  const [resolution, setResolution] = useState('1280x720'); // Default to HD as per requirement
  ...
  const updateActiveFileSetting = (key, value) => {
  ...
  const handleQualityChange = (val) => {
    setQuality(val);
    updateActiveFileSetting('quality', val);
  };

  const handleAudioBitrateChange = (val) => {
    setAudioBitrate(val);
    updateActiveFileSetting('audioBitrate', val);
  };

  const handleResolutionChange = (val) => {
    setResolution(val);
    updateActiveFileSetting('resolution', val);
  };

  const handleAudioTrackChange = (val) => {
    setAudioTrack(val);
    updateActiveFileSetting('audioTrack', val);
  };

  const handlePresetChange = (val) => {
    setPreset(val);
    updateActiveFileSetting('preset', val);
  };
  ...
  const handleAlertOnlyClose = () => {
    ...
    // Reset progress when closing the modal, even if just clicking "X"
    ...
    try {
      if (api.isAvailable()) {
        for (let i = 0; i < selectedFiles.length; i++) {
          ...
          const result = await api.convert('convert-avi-to-h264', {
            sourcePath: file,
            outputDir: outputDir,
            params: {
              quality: settings.quality !== undefined ? settings.quality : quality,
              audioBitrate: settings.audioBitrate !== undefined ? settings.audioBitrate : audioBitrate,
              resolution: settings.resolution !== undefined ? settings.resolution : resolution,
              audioTrack: settings.audioTrack !== undefined ? settings.audioTrack : audioTrack,
              preset: settings.preset !== undefined ? settings.preset : preset,
      ...
      if (!isCancelledRef.current) {
        showAlert('错误', `处理文件时出错: ${err.message}`);
      }
    } finally {
      if (api.isAvailable()) {
    ...
    try {
      ...
      if (Array.isArray(currentTargetsRef.current) && currentTargetsRef.current.length > 0) {
        ...
        if (outputDir) {
          ...
          targetPath = `${outputDir}${sep}${filename}.h264`;
        }
      }
      if (api.isAvailable()) {
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '8px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>AVI To H264 转换器</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px' }}>一款专业工具，可将AVI 视频文件转换为H264(MP4)视频格式，并支持自定义编码参数。</div>
        </div>
      </div>
        ...
        <div className="tool-main-layout">
          <div className="tool-left-panel">
            <FileUploaderV2 
              ...
              showAudioInfo={true}
              uploadPlaceholder="将您的AVI文件拖拽到此处"
            />
            <input 
              type="file" 
              id="file-input" 
          ...
          <div className="tool-right-panel">
            <SettingsPanel title="转换选项">
              <SettingPresets 
                ...
                currentPreset={activeFile && fileSettings[activeFile]?.preset !== undefined ? fileSettings[activeFile].preset : preset}
              ...
              <SettingSlider 
                label="质量" 
                value={activeFile && fileSettings[activeFile]?.quality !== undefined ? fileSettings[activeFile].quality : quality} 
                unit="%" 
                min={1} 
                max={100} 
                step={1}
                onChange={(val) => {
                  ...
                  handlePresetChange('自定义');
                }}
              />

              <LocalSettingSelect 
                label="分辨率" 
                value={activeFile && fileSettings[activeFile]?.resolution !== undefined ? fileSettings[activeFile].resolution : resolution} 
              ...
              <LocalSettingSelect 
                label="音频比特率" 
                value={activeFile && fileSettings[activeFile]?.audioBitrate !== undefined ? fileSettings[activeFile].audioBitrate : audioBitrate} 
                options={bitrateOptions}
                onChange={(val) => {
                  handleAudioBitrateChange(val);
                  handlePresetChange('自定义');
                }}
              />
              ...
              <LocalSettingSelect 
                label="选择音轨" 
                value={activeFile && fileSettings[activeFile]?.audioTrack !== undefined ? fileSettings[activeFile].audioTrack : audioTrack} 
                options={(() => {
                ...
                onChange={(val) => {
                  handleAudioTrackChange(Number(val));
                  handlePresetChange('自定义');
                }}
              />
            </SettingsPanel>
