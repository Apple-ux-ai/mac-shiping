...
const PRESET_CONFIGS = {
  '低质量': { quality: 30, audioBitrate: '96k', resolution: '854:480', useVBR: true },
  ...
  '社交媒体': { quality: 60, audioBitrate: '192k', resolution: '1080:1920', useVBR: true }
};
...
const MOVToWEBMGUI = ({ onBack }) => {
  ...
  // When settings change manually, switch to Custom preset
    ...
    try {
      if (api.isAvailable()) {
        for (let index = 0; index < selectedFiles.length; index++) {
          ...
          const result = await api.convert('convert-mov-to-webm', {
            sourcePath: file,
            outputDir: outputDir,
            params: {
              quality,
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        <div className="tool-breadcrumbs">
          ...
          <span style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <span className="current">MOV To WEBM</span>
          </span>
        </div>
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>MOV To WEBM 转换器</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>—— 专业级 MOV 转 WEBM 工具，支持批量转换和高质量输出。</div>
        </div>
      </div>
          ...
          <div className="tool-right-panel">
            <SettingsPanel title="转换选项">
              ...
              <LocalSettingSelect
                ...
                value={audioTrack}
                options={[
                  { label: '音轨 1 (默认)', value: 0 },
                  ...(fileInfos[activeFile]?.streams?.filter(s => s.codec_type === 'audio').length > 1
                    ? Array.from({ length: fileInfos[activeFile].streams.filter(s => s.codec_type === 'audio').length - 1 }).map((_, i) => ({
                        label: `音轨 ${i + 2}`,
                        value: i + 1
                      }))
                    : [])
                  ...
                  <div className="compression-mode-content" style={{ minHeight: '80px' }}>
                    {useVBR ? (
                      ...
                      <div className="setting-sub-item">
                        ...
                        <select
                          className="custom-select"
                          style={{ width: '100%', height: '36px', borderRadius: '8px', border: '1px solid var(--border-color)', background: 'var(--bg-secondary)', color: 'var(--text-primary)' }}
                          value={videoBitrate}
                          onChange={(e) => {
                    ...
                    <div className="audio-setting-content">
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                        <div className="setting-sub-item">
                          ...
                          <select
                            className="custom-select"
                            style={{ width: '100%', height: '36px', borderRadius: '8px', border: '1px solid var(--border-color)', fontSize: '13px', background: 'var(--bg-secondary)', color: 'var(--text-primary)' }}
