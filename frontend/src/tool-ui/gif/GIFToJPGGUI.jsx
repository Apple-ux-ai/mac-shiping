    ...
    try {
      if (api.isAvailable()) {
        for (let index = 0; index < selectedFiles.length; index++) {
          ...
          const result = await api.convert('convert-gif-to-jpg', {
            sourcePath: file,
            outputDir: outputDir,
            params: {
              quality,
        ...
        if (!isCancelledRef.current) {
          ...
          applyConversionNotificationRule({
            ...
            data: {
              customTitle: '完成',
              customMessage: '所有 GIF 已成功转换为 JPG 序列并打包为 ZIP！',
              buttonText: '确定',
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>GIF To JPG 转换器</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>—— 一款专业工具，可将 GIF 动图转换为 JPG 图像序列，并支持自定义参数。</div>
        </div>
      </div>
