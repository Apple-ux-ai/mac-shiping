    ...
    try {
      if (api.isAvailable()) {
        for (let index = 0; index < selectedFiles.length; index++) {
          ...
          const result = await api.convert('convert-gif-to-avi', {
            sourcePath: file,
            outputDir: outputDir,
            params: {
              fps
        ...
        if (!isCancelledRef.current) {
          ...
          applyConversionNotificationRule({
            ...
            data: {
              customTitle: '完成',
              customMessage: '所有 GIF 已成功转换为 AVI 视频！',
              buttonText: '确定',
              onConfirm: async () => {
                setAlertModal(prev => ({ ...prev, isOpen: false }));
              },
  ...
  const confirmCancel = async () => {
    ...
    if (Array.isArray(currentTargetsRef.current) && currentTargetsRef.current.length > 0) {
      ...
      if (outputDir) {
        ...
        const base = `${outputDir}${sep}${filename}.avi`;
        const candidates = [base];
        for (let i = 1; i <= 50; i++) {
          candidates.push(`${outputDir}${sep}${filename}_${i}.avi`);
        }
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '8px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>GIF To AVI 转换器</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px' }}>一款专业工具，可将 GIF 动图转换为 AVI 视频格式，并支持自定义参数。</div>
        </div>
      </div>
